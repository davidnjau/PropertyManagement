...
import { fetchTenantOverview } from '../../services/tenantPortal'
          ...
          {isLoading ? (
            ...
            <>
              <p className="text-3xl font-extrabold mb-1">
                KES {(data?.rentDue ?? 0).toLocaleString()}
              </p>
              <p className="text-sm text-gray-400">Due {data?.rentDueDate ?? '—'}</p>
            </>
          )}
        </div>
      ...
      {data?.leaseEndDate && (
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-xl px-5 py-4 mb-8">
          ...
          <p className="text-sm text-amber-800">
            Your lease expires on <span className="font-semibold">{data.leaseEndDate}</span>. Contact your agent to discuss renewal.
          </p>
        </div>
      )}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr>
                <td colSpan={4} className="px-5 py-8 text-center text-sm text-gray-400">No payments yet.</td>
              </tr>
            ) : (
